...
rsync -avz --delete \
  ...
  --exclude='.env' \
  C:\Users\Administrador\OneDrive\Documentos\0 SPEEDGROW\one-agent\one-agent-corp\workspace\profitbridge-ai/src/ root@$VPS:$DEPLOY_DIR/src/

# Sync infra files
for f in package.json Dockerfile docker-compose.yml nginx.conf; do
  scp C:\Users\Administrador\OneDrive\Documentos\0 SPEEDGROW\one-agent\one-agent-corp\workspace\profitbridge-ai/src/$f root@$VPS:$DEPLOY_DIR/ 2>/dev/null || true
done
